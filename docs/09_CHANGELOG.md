...
6개 Step, Step별 검증 게이트(tsc·build·E2E) + 사용자 행동 E2E 강제. 마이그 0008·0009 신규.
E2E +30건(step1~5 각 6건), 전체 96 passed / 3 skipped / 0 failed.

- 🆕 **자유 캔버스 폭 확대** (#1) — desktop 캔버스 1200→1680px (40% 확대). mobile 360 유지
- 🆕 **편집 진입 메뉴화** (#4) — 홈 "편집" 버튼 제거 → 햄버거 메뉴 "편집"(`/admin?edit=1`).
  "편집 끝"은 편집 모드 중 홈 상단에 유지
- 🆕 **메모 카드 제목만 표시** (#7) — 홈·공개·위젯의 메모 카드에서 본문 미리보기 제거
- 🆕 **평소모드 안내 이동** (#9) — 홈 상단 "평소 모드(태블릿/PC 레이아웃)" 텍스트 제거 →
  메뉴에 "현재 레이아웃" 표시 행 추가
- 🆕 **글자 크기 pt 전환** (#2·#3, 마이그 0008) — `FontSize` enum(xs~xl) → pt 정수.
  `default_font_size` integer(6~96, 기본 12). 카드별/전역 pt number input + 프리셋
  (9·12·16·20·28pt). 옛 enum 데이터는 `normalizeBlock`/`toPt`로 호환 변환
- 🆕 **메모·URL 카테고리** (#8·#10, 마이그 0009) — 신규 테이블 없이 `mini_homepages`에
  `memo_categories`·`url_categories` JSONB + `memos`·`urls`에 `category_id`.
  `MemosManager`·`UrlsManager`에 카테고리 관리(`CategoryBar`) + 항목별 지정.
  `GET/POST /api/memos·urls/categories`, `PATCH/DELETE .../[id]`
- 🆕 **카드별 표시 카테고리** (#5·#6) — 앨범/메모/URL 카드가 표시할 카테고리를 편집 모드
  floating 컨트롤에서 선택. 앨범 미지정 시 최근 업로드 카테고리. 공개 페이지도 동일 필터

## v0.8 — 2026-05-18 (카드 이름·카테고리 + 그림판 개편)

3개 페이즈, 스텝별 e2e 게이트. 마이그 0007 신규. e2e +24건 (decorate-and-layout/drawing/card-categories 단독 실행 전건 통과).

- 🆕 **모든 카드 이름(헤더) 편집** — `CardHeader` 컴포넌트 신규. 편집 모드에서 프로필·URL·앨범·메모·커스텀·그림 카드의 이름을 직접 수정, 빈 값이면 종류별 기본명. 공개 페이지에도 반영
- 🆕 **카드 카테고리** (마이그 0007) — `mini_homepages.card_categories` JSONB 컬럼(신규 테이블 없음, 6테이블 유지). 추가/삭제 관리 모달, 모든 카드에 카테고리 지정, 헤더 라벨 표시, 편집 모드 카테고리 필터(비일치 카드 흐림), 공개 페이지 라벨 노출. `GET/POST /api/cards/categories`, `DELETE /api/cards/categories/[id]`
- 🆕 **그림판 개편** — `DrawPad.tsx`(225줄)를 `DrawPad/` 모듈로 분리. 펜 5종(형광펜·만년필·연필·볼펜·브러쉬), 도형 6종(직선·사각형·원/타원·삼각형·화살표·별), 픽셀/객체 지우개 2종, 대표색 7 + 커스텀 팔레트, 굵기 1~100 슬라이더. 저장은 기존 PNG 평탄화 경로 유지
- 🐛 **레이아웃 저장 round-trip 복구** — `defaultBlocks` title 카드 높이 70/60 → 80 (`blockSchema` h 최소 80 위반으로 기본 레이아웃 PUT /api/decorate가 400 거부되던 잠재 버그). `handleLayoutsChange`에서 `layoutsRef` 동기 갱신
