...
## v0.9.1 — 캔버스 왼쪽 확장 (2026-05-19)

### 1. 범위
- 사용자 요청 — 가로 왼쪽 제한구역을 8cm(≈302px) 왼쪽으로 확장.
- desktop 캔버스에 `LEFT_PAD=302` 도입. 블록은 `left = x + LEFT_PAD`로 렌더 →
  기존 레이아웃이 통째로 오른쪽으로 밀려 시각 배치 유지, 왼쪽에 302px 여유 구역 확보.
  캔버스 전체 폭 1680 → 1982px. **DB 마이그레이션 불필요**(렌더 오프셋 방식, 저장 좌표 불변).
- 카드 x 하한 0 → -302, 범위 이탈 경고는 콘텐츠 구역(1680) 기준 유지.
...
### 3. 테스트 결과
- 통과 97 / 스킵 3 / 실패 0.
- 신규 사용자 행동 E2E: `step1-ui-layout.spec.ts` TC-S1-007 — 카드를 왼쪽 여유
  구역으로 이동(rendered left 0, 범위 이탈 경고 없음). TC-S1-006은 캔버스 폭
  1982px 기준으로 갱신.
...
### 5. 남은 위험 요소
- 카드 저장 좌표(`block.x`)가 음수가 될 수 있음 — `blockSchema`는 음수 허용,
  렌더는 `x + LEFT_PAD`로 보정. mobile 트랙은 `LEFT_PAD=0`으로 영향 없음.
